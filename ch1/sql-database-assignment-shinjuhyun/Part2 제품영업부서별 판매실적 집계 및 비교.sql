...
select p.product_id, p.product_name, --4 출력 id, nameㅈ ㅓㅇ의 안했으니까 밑에서 gorup해야함
	 count(*) as transaction_count,
	 sum(s.quantity) as total_quantity,
	 sum(s.sale_amount) as total_sales_amount,
	 round(avg(s.sale_amount), 2) as avg_sales_amount --5
	 
	 from sales s --1-1 sale로부터
join products p --1-2 products 연결
	on s.product_id = p.product_id --1-3 제품 ID를 기준으로
	
	where s.sale_date >= '2025-04-01' and s.sale_date < '2025-07-01' --2
		and s.sales_type != '반품' --3
	group by p.product_id, p.product_name --정의
	order by total_sales_amount desc; --총금액순
...
select d.sales_dept_id, d.sales_dept_name, d.sales_region, --얘네 정의 안함 밑에서 group무끽
	count(*) as transaction_count,
	sum(s.quantity) as total_quantity,
	sum(s.sale_amount) as total_sales_amount,
	round(AVG(s.sale_amount), 2) as avg_sales_amount

	from sales s -- 1-1sales로부터
join sales_departments d-- 1-1sales departments연결
	on s.sales_dept_id = d.sales_dept_id -- 1-2 영업부서 ID 기준으로 엮기
	
	where s.sale_date >= '2025-04-01' and s.sale_date < '2025-07-01' -- 2
		and s.sales_type != '반품' --3
group by d.sales_dept_id, d.sales_dept_name, d.sales_region
...
1. 리피다운정 20mg / 호남영업팀
2. 상위 : 호남영업팀 / 하위 : 강원영업팀
3. [호남] 거래 건수 : 26 / 평균 금액 : 1,326,575
   [강원] 거래 건수 : 16 / 평균 금액 : 618,447
-> 호남과 강원의 거래 건수 차이는 10건이나, 평균 금액 차이는 약 2배인 것으로 보아
호남 영업팀이 강원 영업팀에 비하여 건당 판매 금액이 높을 것으로 추정
